...
**Problem 1**
Fit logistic regression model to the `gambia` data in the previous question and use posterior predictive checks to
verify the model fits well. If you find model misspecification, suggest (but do not fit) alternative.


**Solution**

Since we have binary output. We want to test mean of the response. 
```{r, warning=FALSE}
library(geoR)
data(gambia)
```
